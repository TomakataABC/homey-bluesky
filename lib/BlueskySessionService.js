...
class BlueskySessionService {
  ...
  async _getActorProfile(agent, actor) {
    const profile = await agent.getProfile({ actor });
    return profile.data;
  }
  ...
  async isUserMuted({ did, actor }) {
    const { agent } = await this._getAgentForDid(did);
    const state = await this._getRelationshipState(agent, actor);
    return Boolean(state.relationship.muting);
  }

  async isUserBlocked({ did, actor }) {
    const { agent } = await this._getAgentForDid(did);
    const state = await this._getRelationshipState(agent, actor);
    return Boolean(state.relationship.blocking);
  }

  async isFollowedByUser({ did, actor }) {
    const { agent } = await this._getAgentForDid(did);
    const state = await this._getRelationshipState(agent, actor);
    return Boolean(state.relationship.followedBy);
  }

  async isFollowerCountAbove({ actor, threshold }) {
    const handle = String(actor || '').trim();
    if (!handle) return false;
    const serviceUrl = this._normalizeServiceUrl();
    const agent = this._createAgent({ service: serviceUrl, sessionKey: handle });
    const profile = await this._getActorProfile(agent, handle);
    return (profile.followersCount || 0) > Number(threshold || 0);
  }

  async getPostEngagement({ did, postUri }) {
    const { agent } = await this._getAgentForDid(did);
    const post = await this._getPostView(agent, postUri);
    return this._extractCounters(post);
  }
